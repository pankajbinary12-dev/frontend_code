...
const Designations = () => {
  ...
  const fetchDesignations = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/admin/designations/get_desigin', {
        headers: {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/admin/designations/add_desigin', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      await Swal.error('Error!', err.message || 'Failed to add designation')
    }
  }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await Swal.error('Error!', err.message || 'Failed to update designation')
    }
  }

  const handleDelete = async (id) => {
    const result = await Swal.confirm('Are you sure?', 'You want to delete this designation?')
    ...
    if (result.isConfirmed) {
      try {
        ...
        await Swal.error('Error!', err.message || 'Failed to delete designation')
      }
    }
  }
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
      ...
      <p style={{ marginTop: '20px', fontSize: '18px', color: '#667eea', fontWeight: '600' }}>Loading Designations...</p>
    </div>
  )
    ...
    return (
      <div className="module-container">
        <div className="error-message">
          <p>⚠️ Error: {error}</p>
          <button className="btn-primary" onClick={fetchDesignations} style={{ marginTop: '20px' }}>🔄 Retry</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="module-container">
      ...
      <div className="pagination-info">
        Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filtered.length)} of {filtered.length} designations
      </div>

      {/* View Modal */}
      <Modal isOpen={viewModal} onClose={() => setViewModal(false)} title="Designation Details">
        {selectedItem && (
      ...
      {/* Add Modal */}
      <Modal isOpen={addModal} onClose={() => setAddModal(false)} title="Add Designation">
      ...
      {/* Edit Modal */}
      <Modal isOpen={editModal} onClose={() => setEditModal(false)} title="Edit Designation">
