...
const Sections = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/admin/sections/store_section', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok && data.success) {
        await Swal.success('Success!', data.message || 'Section added successfully')
        setAddModal(false)
        setFormData({ name: '', description: '' })
        fetchSections()
      } else {
        await Swal.error('Error!', data.message || data.error || 'Failed to add section')
      }
    } catch (err) {
      console.error('Add Error:', err)
      await Swal.error('Error!', err.message || 'Failed to add section')
    }
  }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/admin/sections/update_section_by_id/${selectedItem.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok && data.success) {
        await Swal.success('Success!', data.message || 'Section updated successfully')
        setEditModal(false)
        fetchSections()
      } else {
        await Swal.error('Error!', data.message || data.error || 'Failed to update section')
      }
    } catch (err) {
      console.error('Update Error:', err)
      await Swal.error('Error!', err.message || 'Failed to update section')
    }
  }

  const handleDelete = async (id) => {
    const result = await Swal.confirm('Are you sure?', 'You want to delete this section?')
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(`http://127.0.0.1:8000/api/admin/sections/delete_section_by_id/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        if (response.ok && data.success) {
          await Swal.success('Deleted!', data.message || 'Section deleted successfully')
          fetchSections()
        } else {
          await Swal.error('Error!', data.message || data.error || 'Failed to delete section')
        }
      } catch (err) {
        console.error('Delete Error:', err)
        await Swal.error('Error!', err.message || 'Failed to delete section')
      }
    }
  }
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
      ...
      <p style={{ marginTop: '20px', fontSize: '18px', color: '#667eea', fontWeight: '600' }}>Loading Sections...</p>
    </div>
  )
    ...
    return (
      <div className="module-container">
        <div className="error-message">
          <p>⚠️ Error: {error}</p>
          <button className="btn-primary" onClick={fetchSections} style={{ marginTop: '20px' }}>🔄 Retry</button>
        </div>
      </div>
    )
  }
    ...
    <div className="module-container">
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {current.length === 0 ? (
              <tr><td colSpan="5" style={{ textAlign: 'center', padding: '20px' }}>No sections found</td></tr>
            ) : (
              current.map((item) => (
                <tr key={item.id}>
                  <td>{item.id}</td>
      ...
      <div className="pagination-info">
        Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filtered.length)} of {filtered.length} sections
      </div>

      {/* View Modal */}
      <Modal isOpen={viewModal} onClose={() => setViewModal(false)} title="Section Details">
        {selectedItem && (
      ...
      <Modal isOpen={addModal} onClose={() => setAddModal(false)} title="Add Section">
        <form className="data-form" onSubmit={handleAdd}>
          <div className="form-group">
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required placeholder="e.g., A, B, C, D" />
          </div>
          <div className="form-group">
      ...
      {/* Edit Modal */}
      <Modal isOpen={editModal} onClose={() => setEditModal(false)} title="Edit Section">
        <form className="data-form" onSubmit={handleUpdate}>
          ...
          <div className="form-actions">
            <button type="submit" className="btn-primary">Update Section</button>
