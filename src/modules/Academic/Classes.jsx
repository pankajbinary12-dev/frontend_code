...
const Classes = () => {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/admin/classes/store_class', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok && data.success) {
        await Swal.success('Success!', data.message || 'Class added successfully')
        setAddModal(false)
        setFormData({ name: '', description: '' })
        fetchClasses()
      } else {
        await Swal.error('Error!', data.message || data.error || 'Failed to add class')
      }
    } catch (err) {
      console.error('Add Error:', err)
      await Swal.error('Error!', err.message || 'Failed to add class')
    }
  }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/admin/classes/update_class_by_id/${selectedItem.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok && data.success) {
        await Swal.success('Success!', data.message || 'Class updated successfully')
        setEditModal(false)
        fetchClasses()
      } else {
        await Swal.error('Error!', data.message || data.error || 'Failed to update class')
      }
    } catch (err) {
      console.error('Update Error:', err)
      await Swal.error('Error!', err.message || 'Failed to update class')
    }
  }

  const handleDelete = async (id) => {
    const result = await Swal.confirm('Are you sure?', 'You want to delete this class?')
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(`http://127.0.0.1:8000/api/admin/classes/delete_class_by_id/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
        ...
        if (response.ok && data.success) {
          await Swal.success('Deleted!', data.message || 'Class deleted successfully')
          fetchClasses()
        } else {
          await Swal.error('Error!', data.message || data.error || 'Failed to delete class')
        }
      } catch (err) {
        console.error('Delete Error:', err)
        await Swal.error('Error!', err.message || 'Failed to delete class')
      }
    }
  }
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
      ...
      <p style={{ marginTop: '20px', fontSize: '18px', color: '#667eea', fontWeight: '600' }}>Loading Classes...</p>
    </div>
  )
    ...
    return (
      <div className="module-container">
        <div className="error-message">
          <p>⚠️ Error: {error}</p>
          <button className="btn-primary" onClick={fetchClasses} style={{ marginTop: '20px' }}>🔄 Retry</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="module-container">
      ...
      <div className="pagination-info">
        Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filtered.length)} of {filtered.length} classes
      </div>

      {/* View Modal */}
      <Modal isOpen={viewModal} onClose={() => setViewModal(false)} title="Class Details">
        {selectedItem && (
      ...
      {/* Add Modal */}
      <Modal isOpen={addModal} onClose={() => setAddModal(false)} title="Add Class">
        <form className="data-form" onSubmit={handleAdd}>
          <div className="form-group">
            <label>Class Name *</label>
            <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required placeholder="e.g., I, II, III, IV" />
          </div>
          <div className="form-group">
      ...
      {/* Edit Modal */}
      <Modal isOpen={editModal} onClose={() => setEditModal(false)} title="Edit Class">
